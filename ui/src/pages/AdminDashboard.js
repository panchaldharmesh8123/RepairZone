...
function AdminDashboard() {
  ...
  const handleSaveUser = async (e) => {
    ...
    try {
      if (currentUserData.id) {
        // Update user
        await apiCall(
          `/api/admin/users/${currentUserData.id}`,
          "PUT",
          currentUserData,
          localStorage.getItem("token")
        );
  ...
  const handleEditWorker = (worker) => {
    setCurrentWorkerData({
      ...
      email: worker.email,
      password: "",
      availableServices: worker.availableServices.map(
        (s) => s._id || s.toString()
      ),
    });
    setShowWorkerForm(true);
  ...
  return (
    <div className="card shadow-sm p-4">
      ...
      <ul className="nav nav-tabs mb-4">
        ...
        <li className="nav-item">
          <button
            className={`nav-link ${tab === "workers" ? "active" : ""}`}
            ...
            <div>
              ...
              {showUserForm && (
                <div className="card mb-3 p-3">
                  <h6 className="mb-3">
                    {currentUserData.id ? "Edit User" : "Add User"}
                  </h6>
          ...
          {tab === "services" && (
            <div>
              ...
              {showServiceForm && (
                <div className="card mb-3 p-3">
                  <h6 className="mb-3">
                    {currentServiceData.id ? "Edit Service" : "Add Service"}
                  </h6>
          ...
          {tab === "workers" && (
            <div>
              ...
              {showWorkerForm && (
                <div className="card mb-3 p-3">
                  <h6 className="mb-3">
                    {currentWorkerData.id ? "Edit Worker" : "Add Worker"}
                  </h6>
                      ...
                      {services.map((s) => (
                        <div className="form-check" key={s._id}>
                          <input
                            ...
                            type="checkbox"
                            value={s._id}
                            checked={currentWorkerData.availableServices.includes(
                              s._id
                            )}
                            onChange={(e) => {
                              ...
                              setCurrentWorkerData((prevData) => ({
                                ...
                                availableServices: e.target.checked
                                  ...
                                  : prevData.availableServices.filter(
                                      (id) => id !== serviceId
                                    ),
                              }));
                            }}
                    ...
                    <tbody>
                      {workers.map((w) => (
                        <tr key={w._id}>
                          ...
                          <td>{w.email}</td>
                          <td>
                            {w.availableServices.map((s) => s.name).join(", ")}
                          </td>
                          <td>
                            <button
                              className="btn btn-info btn-sm me-2"
